...
// part of the AI threat response system (dashboard, timeline, findings, rules, reports) lights up
  ...
  .handler(async ({ context }) => {
    ...
    const SCENARIOS = [
      {
        ...
        ai_category: "Suspicious Login",
        title: "Root console login from new geo (Belarus)",
        ai_summary:
          "Root user logged in from an IP geolocated to Minsk, BY — first time this account has authenticated from that ASN. MFA was not used.",
        ai_remediation:
          "Force a root password rotation, enforce MFA, and review all actions performed by this session in the last hour.",
        raw: {
          eventName: "ConsoleLogin",
          userIdentity: { type: "Root", userName: "root", accessKeyId: "ASIATESTROOTKEY01" },
      ...
      },
      {
        ...
        source_ip: "203.0.113.42",
        ai_severity: "critical",
        ai_category: "Persistence",
        title: "New long-lived access key minted for high-privilege IAM user",
        ai_summary:
          "A new programmatic access key was created for IAM user 'alice.dev' (AdministratorAccess) from the same IP that performed the suspicious console login 4 minutes earlier.",
        ai_remediation:
          "Disable the new key, rotate alice.dev's credentials, and audit CloudTrail for any API calls that used the new key.",
        raw: {
          eventName: "CreateAccessKey",
          userIdentity: { userName: "alice.dev", accessKeyId: "AKIATESTALICE0001" },
          requestParameters: { userName: "alice.dev" },
          responseElements: { accessKey: { accessKeyId: "AKIATEST0002COMPRO" } },
        },
      },
      {
        source: "guardduty",
        event_name: "UnauthorizedAccess:IAMUser/ConsoleLoginSuccess.B",
        username: "alice.dev",
        source_ip: "203.0.113.42",
        ai_severity: "high",
        ai_category: "Suspicious Login",
        ...
        ai_summary:
          "GuardDuty flagged this login as inconsistent with the IAM user's historical behavior (new ASN, new user agent, off-hours).",
        ai_remediation:
          "Correlate with the matching CloudTrail event; if not legitimate, suspend the user and revoke active sessions.",
        raw: {
          Id: "gd-mock-001",
          Type: "UnauthorizedAccess:IAMUser/ConsoleLoginSuccess.B",
          Severity: 8,
          Resource: {
            AccessKeyDetails: { UserName: "alice.dev", AccessKeyId: "AKIATESTALICE0001" },
          },
        },
      },
      {
        ...
        source_ip: "203.0.113.42",
        ai_severity: "critical",
        ai_category: "Defense Evasion",
        title: "CloudTrail logging stopped on primary audit trail",
        ai_summary:
          "The 'org-audit-trail' was disabled, blinding detection in this region. Classic anti-forensics step in a kill chain.",
        ai_remediation:
          "Immediately re-enable the trail, lock the trail with an SCP that denies cloudtrail:StopLogging, and treat the account as compromised.",
        raw: {
          eventName: "StopLogging",
          requestParameters: { name: "org-audit-trail" },
          userIdentity: { userName: "alice.dev" },
        },
      },
      {
        source: "cloudtrail",
        ...
        source_ip: "203.0.113.42",
        ai_severity: "high",
        ai_category: "Network Exposure",
        title: "Security group opened SSH (22) to 0.0.0.0/0",
        ai_summary:
          "Inbound rule added to sg-prod-app allowing SSH from the entire internet — high blast radius for compromised production hosts.",
        ai_remediation:
          "Revoke the rule, restrict to known bastion CIDRs, and require approval workflows for SG changes via SCP.",
        raw: {
      ...
      },
      {
        ...
        source_ip: "203.0.113.42",
        ai_severity: "high",
        ai_category: "Data Exfiltration",
        title: "Bulk Secrets Manager reads (32 secrets in 90s)",
        ai_summary:
          "Same identity enumerated and read 32 secrets in rapid succession, including 'prod/db/master' and 'stripe/live'.",
        ai_remediation:
          "Rotate every secret accessed in that window and add anomaly detection on secretsmanager:GetSecretValue volume.",
        raw: {
          eventName: "GetSecretValue",
          requestParameters: {
            secretId: "arn:aws:secretsmanager:us-east-1:" + acct + ":secret:prod/db/master",
          },
        },
      },
      {
        ...
        source_ip: "10.0.5.12",
        ai_severity: "medium",
        ai_category: "Misconfiguration",
        title: "S3 bucket policy widened to AllowAll Principal",
        ai_summary:
          "Bucket policy on 'analytics-exports' now permits s3:GetObject for Principal '*' without a Condition block.",
        ai_remediation:
          "Restore the previous policy and require a least-privilege Condition (aws:SourceVpce or aws:PrincipalOrgID).",
        raw: {
          eventName: "PutBucketPolicy",
          requestParameters: { bucketName: "analytics-exports" },
        },
      },
      {
        source: "cloudtrail",
        ...
        raw: {
          ...
          requestParameters: { roleArn: `arn:aws:iam::${acct}:role/ci-deployer` },
        },
      },
      {
        source: "cloudtrail",
        ...
        ai_summary:
          "IAM user was created directly via the console rather than the Terraform pipeline — does not match any active change ticket.",
        ai_remediation:
          "Delete the user, restrict iam:CreateUser to the deploy role, and require change-ticket annotation in CloudTrail.",
        raw: { eventName: "CreateUser", requestParameters: { userName: "backup-svc" } },
      },
      {
        source: "guardduty",
        event_name: "Recon:IAMUser/MaliciousIPCaller.Custom",
        username: "backup-svc",
        ...
        ai_summary:
          "The newly created IAM user immediately enumerated permissions from an IP listed in a threat-intel feed.",
        ai_remediation:
          "Disable backup-svc, block the source IP at the network edge, and review attached policies.",
      ...
      const ruleRows = [
        {
          ...
          description: "Any console login by the root account is critical.",
          mitre_technique: "T1078.004",
          severity: "critical",
          match_event_names: ["ConsoleLogin"],
          match_keywords: ["Root"],
        },
        {
          name: "CloudTrail tampering",
          description: "Disabling or deleting trails is a defense-evasion signal.",
          mitre_technique: "T1562.008",
          severity: "critical",
          match_event_names: ["StopLogging", "DeleteTrail"],
          match_keywords: [],
        },
        {
          name: "Public security group ingress",
          description: "Inbound 0.0.0.0/0 on sensitive ports.",
          mitre_technique: "T1133",
          severity: "high",
          match_event_names: ["AuthorizeSecurityGroupIngress"],
          match_keywords: ["0.0.0.0/0"],
        },
        {
          name: "Bulk Secrets Manager reads",
          description: "Rapid enumeration of secrets values.",
          mitre_technique: "T1552.007",
          severity: "high",
          match_event_names: ["GetSecretValue"],
          match_keywords: [],
        },
        {
          name: "Out-of-band IAM user creation",
          description: "IAM users created outside the deploy pipeline.",
          mitre_technique: "T1136.003",
          severity: "high",
          match_event_names: ["CreateUser"],
          match_keywords: [],
        },
      ].map((r) => ({
        ...r,
    ...
    if (insertedIds.length) {
      await supabaseAdmin.from("incident_reports").insert({
        ...
        severity: "critical",
        executive_summary:
          "An attacker authenticated as root from a previously-unseen geography, minted new long-lived access keys, disabled CloudTrail, opened production SSH to the internet, and bulk-read Secrets Manager. The chain has the hallmarks of credential-stuffing followed by hands-on-keyboard escalation.",
        mitre_tactics: ["TA0001", "TA0003", "TA0005", "TA0006", "TA0010"],
        affected_resources: [
          `arn:aws:iam::${acct}:user/alice.dev`,
          "org-audit-trail",
          "sg-prod-app",
...
export const clearMockData = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context }) => {
    ...
    await supabaseAdmin
      ...
      .delete()
      .eq("user_id", userId)
      .eq("generated_by", "sandbox");
    await supabaseAdmin
      .from("aws_connections")
      .update({ mock_data_seeded_at: null })
      .eq("user_id", userId);
  ...
  .handler(async ({ data, context }) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        return {
          ok: false as const,
          error: `Elasticsearch returned ${res.status}: ${t.slice(0, 200)}`,
        };
      }
      ...
      if (!conn)
        return {
          ok: false as const,
          error: "Connect AWS first — Elasticsearch attaches to a connection.",
        };
...
export const syncFromElasticsearch = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context }) => {
    ...
    let apiKey: string;
    try {
      apiKey = decryptSecret(conn.es_api_key_encrypted);
    } catch {
      return { ok: false as const, error: "Cannot decrypt ES key — reconnect cluster" };
    }
    ...
    const res = await fetch(
      `${conn.es_endpoint}/${encodeURIComponent(conn.es_index ?? "cloudtrail-*")}/_search`,
      {
        method: "POST",
