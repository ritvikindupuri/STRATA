  ...
  if (loading || !user) {
    return (
      <div className="flex min-h-screen items-center justify-center">
        <div className="font-mono text-sm text-muted-foreground animate-shimmer">
          // initializing strata…
        </div>
      </div>
    );
    ...
    <div className="flex min-h-screen">
      <aside className="hidden w-64 shrink-0 flex-col border-r border-border bg-card/40 md:flex">
        ...
        <div className="border-t border-border p-3">
          <div className="rounded-md px-3 py-2 text-xs text-muted-foreground font-mono truncate">
            {user.email}
          </div>
          <button
            onClick={async () => {
            ...
            className="mt-2 flex w-full items-center gap-2 rounded-md px-3 py-2 text-sm text-muted-foreground hover:bg-card hover:text-foreground"
          >
