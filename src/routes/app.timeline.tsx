...
function TimelinePage() {
  const events = useQuery({
    ...
    queryFn: async () => {
      const [actions, findings, reports] = await Promise.all([
        supabase.from("agent_actions").select("*").order("created_at", { ascending: false }).limit(80),
        supabase.from("findings").select("id,title,event_name,event_time,ai_severity,ai_category,source,username,source_ip,created_at").in("ai_severity", ["high","critical"]).order("created_at", { ascending: false }).limit(80),
        supabase.from("incident_reports").select("id,title,severity,created_at,executive_summary").order("created_at", { ascending: false }).limit(20),
      ]);
      const items: any[] = [];
      for (const a of actions.data ?? []) items.push({ kind: "action", ts: a.created_at, data: a });
      for (const f of findings.data ?? []) items.push({ kind: "finding", ts: f.event_time ?? f.created_at, data: f });
  ...
  return (
    <div className="p-8">
      <p className="font-mono text-xs uppercase tracking-widest text-primary">// incident timeline</p>
      <h1 className="mt-1 font-display text-3xl font-semibold">Live event ledger</h1>
      <p className="mt-2 text-sm text-muted-foreground">Every detection, agent action, and incident report — in order.</p>

      <div className="mt-8 relative">
        <div className="absolute left-[14px] top-2 bottom-2 w-px bg-gradient-to-b from-primary/40 via-border to-transparent" />
        <ul className="space-y-4">
          {items.map((it, idx) => <TimelineItem key={idx} item={it} />)}
          {!items.length && (
...
function TimelineItem({ item }: { item: any }) {
  ...
  let Icon = ICON.default, color = "var(--primary)", title = "", desc = "", tag = "";
  ...
  if (kind === "action") {
    ...
    color = data.status === "failed" ? "var(--critical)" : data.status === "skipped" ? "var(--muted-foreground)" : "var(--success)";
  ...
  return (
    <li className="relative pl-12">
      <div className="absolute left-0 top-1 grid h-7 w-7 place-items-center rounded-full border bg-background"
      ...
      <div className="rounded-lg border border-border bg-card/50 p-4 card-elevated">
        <div className="flex flex-wrap items-center gap-2">
          ...
          <span className="rounded-full border border-border px-2 py-0.5 font-mono text-[10px] uppercase tracking-wider text-muted-foreground">{tag}</span>
        </div>
        {desc && <p className="mt-1 text-sm text-muted-foreground">{desc}</p>}
        <p className="mt-2 font-mono text-[10px] text-muted-foreground/70" title={format(new Date(ts), "PPpp")}>
